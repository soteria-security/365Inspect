...
function Inspect-AdminUsersWithNoMFAEnforced {
    Try {
        ...
        If ($tenantLicense.ServicePlanName -match "AAD_PREMIUM*") {
            ...
            $adminMFACAPolicies = (Invoke-GraphRequest -method get -uri "https://graph.microsoft.com/beta/policies/conditionalAccessPolicies").Value | Where-Object { ($_.state -eq "Enabled") -and ($_.conditions.users.includeroles) -and ($_.grantcontrols.builtincontrols -eq "Mfa") }
            ...
            If (($secureDefault.IsEnabled -eq $false) -and (-NOT $MFAviaCA)) {
                $adminRoles = (Invoke-GraphRequest -method get -uri "https://graph.microsoft.com/beta/directoryRoles").Value | Where-Object { $_.DisplayName -like "*Admin*" }
        
                $mfaReport = ((Invoke-GraphRequest -method get -uri "https://graph.microsoft.com/beta/reports/credentialUserRegistrationDetails").Value | Where-Object { $_.isMfaRegistered -eq $false }).userPrincipalName
        
                $results = @()
        
                ForEach ($role in $adminRoles) {
                    $roleMembers = @()
                    $roleMembers = (Invoke-GraphRequest -method get -uri "https://graph.microsoft.com/beta/roleManagement/directory/roleAssignments").Value 
