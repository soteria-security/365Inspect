...
Function Get-TenantLicenseLevel{
    $Licenses = (Get-MsolAccountSku | Where-Object {$_.AccountSkuId -like "*"}).ServiceStatus.ServicePlan

    $org_licenses = @()

    foreach ($license in $Licenses.ServicePlanId){
        $org_licenses += $MicrosoftSKUS.Item("$license")
    }
