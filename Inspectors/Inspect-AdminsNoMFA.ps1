...
function Inspect-AdminUsersWithNoMFAEnforced {
    Try {
        ...
        If ($tenantLicense.ServicePlanName -match "AAD_PREMIUM*") {
            ...
            $adminMFACAPolicies = (Invoke-GraphRequest -method get -uri "https://$(@($global:graphURI))/beta/policies/conditionalAccessPolicies").Value | Where-Object { ($_.state -eq "Enabled") -and ($_.conditions.users.includeroles) -and ($_.grantcontrols.builtincontrols -eq "Mfa") }
            ...
            If (($secureDefault.IsEnabled -eq $false) -and (-NOT $MFAviaCA)) {
                $adminRoles = (Invoke-GraphRequest -method get -uri "https://$(@($global:graphURI))/beta/directoryRoles").Value | Where-Object { $_.DisplayName -like "*Admin*" }
        
                $mfaReport = ((Invoke-GraphRequest -method get -uri "https://$(@($global:graphURI))/beta/reports/credentialUserRegistrationDetails").Value | Where-Object { $_.isMfaRegistered -eq $false }).userPrincipalName
        
                $results = @()
        
                ForEach ($role in $adminRoles) {
                    $roleMembers = @()
                    $roleMembers = (Invoke-GraphRequest -method get -uri "https://$(@($global:graphURI))/beta/roleManagement/directory/roleAssignments").Value 
                    ...
                    If ($roleMembers.count -gt 0) {
                        Foreach ($member in $roleMembers) {
                            If ($member.roleDefinitionId -eq $role.roleTemplateId) {
                                $entity = (Invoke-GraphRequest -method get -uri "https://$(@($global:graphURI))/beta/directoryObjects/$($member.principalId)")
        
                                If ($entity.'@odata.type' -eq '#microsoft.graph.user') {
                                    $users += $entity.userPrincipalName
                                }
                                If ($entity.'@odata.type' -eq '#microsoft.graph.group') {
                                    $groupMembers = (Invoke-GraphRequest -method get -uri "https://$(@($global:graphURI))/beta/groups/$(($entity).Id)/members").Value
