...
Function Inspect-AADRoles {
    Try {
        ...
        $definitions = (Invoke-GraphRequest -method get -uri "https://microsoftgraph.chinacloudapi.cn/beta/roleManagement/directory/roleDefinitions").Value
            
        Foreach ($definition in $definitions) {
            $assignments = (Invoke-GraphRequest -method get -uri "https://microsoftgraph.chinacloudapi.cn/beta/roleManagement/directory/roleAssignments").Value | Where-Object { $_.roleDefinitionId -eq $definition.id }
            
            foreach ($assignment in $assignments) {
                $objects = (Invoke-GraphRequest -method get -uri "https://microsoftgraph.chinacloudapi.cn/beta/directoryObjects/$($assignment.principalId)")
