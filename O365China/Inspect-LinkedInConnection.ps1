...
Function Inspect-LinkedInConnection {
    Try {
        $result = (Invoke-GraphRequest -Method GET -Uri "https://microsoftgraph.chinacloudapi.cn/beta/servicePrincipals?filter=displayname eq 'O365 LinkedIn Connection'").Value
        ...
        ElseIf ($result.accountEnabled -and ($null -ne $result.tags)) {
            ...
            $group = (Invoke-GraphRequest -Method GET -Uri "https://microsoftgraph.chinacloudapi.cn/beta/groups?filter=id eq '$grpAssigned'&select=displayName").Value.displayName
