...
Function Inspect-ServicePrincipalKeyCredentials {
    Try {
        $results = @()

        $servicePrincipals = @()

        $query = (Invoke-GraphRequest -Method Get -Uri "https://microsoftgraph.chinacloudapi.cn/beta/servicePrincipals")
        ...
        $servicePrincipals = $servicePrincipals | Where-Object { $_.keyCredentials.KeyId }
...
Return Inspect-ServicePrincipalKeyCredentials