...
Function Inspect-DirSyncAdmins {
    Try {
        ...
        If ($syncEnabled -eq $true) {
            $adminRoles = (Invoke-GraphRequest -method get -uri "https://microsoftgraph.chinacloudapi.cn/beta/directoryRoles").Value | Where-Object { $_.displayName -match "Administrator" }

            foreach ($role in $adminRoles) {
                $members = (Invoke-GraphRequest -method get -uri "https://microsoftgraph.chinacloudapi.cn/beta/directoryRoles/$($role.id)/members").Value
