    ...
    Try {
        ...
        If ($tenantLicense.ServicePlanName -match "AAD_PREMIUM*") {
            ...
            ElseIf (($secureDefault.IsEnabled -eq $false) -and ($conditionalAccess.count -eq 0)) {
                ...
                $policies = ((Invoke-GraphRequest -method get -uri "https://graph.microsoft.us/beta/policies/conditionalAccessPolicies").Value | Where-Object { ($null -ne $_.conditions.userRiskLevels) -and (($_.conditions.userRiskLevels -eq 'high') -or ($_.conditions.userRiskLevels -eq 'medium')) })
