...
function Inspect-NoMFA {
    Try {
        ...
        If ($tenantLicense.ServicePlanName -match "AAD_PREMIUM*") {
            $MFAviaCA = (Invoke-GraphRequest -method get -uri "https://graph.microsoft.us/beta/policies/conditionalAccessPolicies").Value | Where-Object { ($_.state -eq "Enabled") -and ($_.conditions.users.includeusers -eq "All") -and ($_.grantcontrols.builtincontrols -eq "Mfa") }
            
            If (($secureDefault.IsEnabled -eq $false) -and (-NOT $MFAviaCA)) {
                $unenforced_users += (Invoke-GraphRequest -method get -uri "https://graph.microsoft.us/beta/reports/credentialUserRegistrationDetails").Value | Where-Object { $_.isMfaRegistered -eq $false }
            }
