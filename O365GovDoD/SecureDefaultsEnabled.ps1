...
function Inspect-SecureDefaults {
    Try {
        $conditionalAccess = (Invoke-GraphRequest -method get -uri "https://dod-graph.microsoft.us/beta/policies/conditionalAccessPolicies" -ErrorAction Stop).Value

        If (($conditionalAccess | Measure-Object).Count -eq 0) {
            $SDCreationDate = "October 22, 2019"
            $tenantCreationDate = (Invoke-GraphRequest -Method Get -Uri "https://dod-graph.microsoft.us/beta/organization?select=createdDateTime").Value.CreatedDateTime
            $secureDefault = (Invoke-GraphRequest -method get -uri "https://dod-graph.microsoft.us/beta/policies/identitySecurityDefaultsEnforcementPolicy" -ErrorAction Stop).IsEnabled
