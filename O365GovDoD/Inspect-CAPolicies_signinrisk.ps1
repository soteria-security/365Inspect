...
function Inspect-CAPolicies_signinrisk {
    Try {
        ...
        If ($tenantLicense.ServicePlanName -match "AAD_PREMIUM_P2") {
            ...
            $conditionalAccess = (Invoke-GraphRequest -method get -uri "https://dod-graph.microsoft.us/beta/policies/conditionalAccessPolicies").Value
            ...
            ElseIf (($secureDefault.IsEnabled -eq $false) -and ($conditionalAccess.count -eq 0)) {
                ...
                $policies = ((Invoke-GraphRequest -method get -uri "https://dod-graph.microsoft.us/beta/policies/conditionalAccessPolicies").Value | Where-Object { ($null -ne $_.conditions.signinRiskLevels) -and (($_.conditions.signinRiskLevels -eq 'high') -or ($_.conditions.signinRiskLevels -eq 'medium')) })
