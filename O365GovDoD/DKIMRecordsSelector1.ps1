...
function Inspect-DKIMRecordsSelector1 {
    Try {
        $domains = (Invoke-GraphRequest -method get -uri "https://dod-graph.microsoft.us/beta/domains" -ErrorAction Stop).Value | Where-Object { $_.Id -notlike "*.*microsoft*.com" }
        $domains_without_records = @()
	
        ForEach ($domain in $domains.Id) {
            Try {
		    ($dkim_one_output = (Resolve-DnsName -Name "selector1._domainkey.$domain" -Type CNAME -Server 8.8.8.8 -ErrorAction Stop).Name) | Out-Null
            }
            Catch {
